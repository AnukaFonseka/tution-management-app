...
export const fetchAssignmentTypes = async () => {
  try {
    const { data, error } = await supabase
...
export const fetchAllAssignmentTypes = async () => {
  try {
    const { data, error } = await supabase
      .from('assignment_types')
      .select('*')
    ...
    console.error('Error fetching all assignment types:', error);
    return [];
  }
};
...
export function getGradeLabels(gradeNumbers) {
  return gradeNumbers.map(num => 
    GRADES.find(grade => grade.value === num.grades.name)?.label || `Grade ${num.grades.name}`
  )
}
...
// Helper function to convert grade names to IDs for database operations
export const getGradeIdsByNames = async (gradeNames) => {
  try {
    const { data, error } = await supabase
      .from('grades')
      .select('id, name')
    ...
    console.error('Error fetching grade IDs:', error);
    return [];
  }
};

// Helper function to get grade names by IDs
export const getGradeNamesByIds = async (gradeIds) => {
  try {
    const { data, error } = await supabase
      .from('grades')
      .select('id, name')
    ...
    if (error) throw error;
    return data.map(grade => grade.name);
  } catch (error) {
